...
class TriggerMatchingServiceTest {
    ...
    class ConditionOnlyTriggerTests {
        ...
        @Test
        void removeConditionOnlyTriggers() {
            UUID protDefId = UUID.randomUUID();
            service.registerConditionOnlyTriggers(protDefId, List.of(
                    new ConditionOnlyTrigger(protDefId, "action-1", "text/jsonlogic", "{\"==\":[1,1]}")
            ));

            assertEquals(1, service.getConditionOnlyTriggers().size());

            service.removeConditionOnlyTriggers(protDefId);
        ...
        void removeNonexistent_noError() {
            assertDoesNotThrow(() -> service.removeConditionOnlyTriggers(UUID.randomUUID()));
        }
        ...
        @Test
        void removeOnlyAffectsTargetProtocol() {
            UUID protDefId1 = UUID.randomUUID();
            UUID protDefId2 = UUID.randomUUID();
            ...
            service.registerConditionOnlyTriggers(protDefId2, List.of(
                    new ConditionOnlyTrigger(protDefId2, "action-2", "text/jsonlogic", "{\"==\":[2,2]}")
            ));

            service.removeConditionOnlyTriggers(protDefId1);

            List<ConditionOnlyTrigger> remaining = service.getConditionOnlyTriggers();
            assertEquals(1, remaining.size());
            assertEquals(protDefId2, remaining.get(0).protocolDefinitionId());
        }
    }
}
