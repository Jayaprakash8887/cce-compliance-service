...
- `V4__state_history.sql` adds append-only `protocol_instance_history` + `step_instance_history` tables and installs `AFTER INSERT OR UPDATE` triggers that record every `status`/`state` transition (12 total). A one-time seed of existing rows is included but **disabled by default** (enable only for re-snapshot backfill of pre-V4 rows). CDC replica identity / publication / grants are configured centrally in the data-pipeline (`cdc/01-configure-replication.sql`), not in this migration. See the [State-Transition History tables](data-dictionary.md#13-state-transition-history-tables) in the data dictionary.
