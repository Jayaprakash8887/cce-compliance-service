...
| `type` | Yes | String | Event type — must follow `org.openphc.cce.<resource>` pattern (e.g., `org.openphc.cce.observation`). Validated by Collector; emitter adaptors are responsible for normalization. |
...
|---|---|---|
| `org.openphc.cce.<resource>` | `org.openphc.cce.observation` | Clinical resource event (validated by Collector; emitter adaptors handle normalization) |
| `org.openphc.cce.compliance.deviation.*` | `org.openphc.cce.compliance.deviation.overdue` | Compliance-generated deviation events |

> **Note:** The Collector Service **strictly validates** that inbound `type` matches the `org.openphc.cce.<resource>` pattern. Event type normalization is the responsibility of the emitter adaptor (openHIM mediator layer). The Compliance Service can rely on all events having correctly formatted types.
